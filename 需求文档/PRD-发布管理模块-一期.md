...
> **文档版本**：v1.19 · 2026-06（PRD 修订号，见文末变更说明）  
> 关联文档：《一期功能清单》《平台功能列表-一期》《PRD-Prompt 评测-一期》  
> **文件路径**：`AI 译评工作台/需求文档/PRD-发布管理模块-一期.md`  
> **交互原型**：`AI 译评工作台/eval-prototype/`（发布管理区块已合并；`prototype/` 跳转）
...
**发布管理模块**一期是**全新建设**，用于把 Prompt 从代码工程中解耦：管理语种生命周期、维护 Prompt 版本库、在**测试 / 预发 / 正式**三环境绑定并发布 Prompt，并通过配置 API 供 AI 译配运行时拉取。**一期不做登录与权限**（已确认），面向内网/VPN 可信环境。  
**产品形态**：侧栏含 **Prompt 评测** / **人校评阅**（占位）/ **发布管理**（`#/config`）；本 PRD 覆盖 **发布管理**。**配置服务** 与 **迭代服务** 分库分部署。
...
| 角色 | 姓名 | 职责 |
|------|------|------|
| 产品 | 张小飞、李姝宜 | 需求、验收、发布流程拍板 |
| 管理平台 | 张小飞、李姝宜 | 平台 Owner、排期 |
| 译配接入 | 张雨时、张健 | 测试/预发/正式配置化接入 |
| 译评研发 | 蒙则韶 | 配置服务与发布管理 Web + Web + 配置 API |
...
发布管理模块定位为 **Prompt 与语种配置的单一事实来源（SSOT）**：

- 译评：写 Prompt、评测、提交**过线候选**  
- 发布管理模块：入库、绑定环境、发布/回滚  
- 译配：按环境读配置，不再硬编码 Prompt  

### 3.2 为什么现在做

1. 新语种 SOP 阶段 3 明确 **发布管理 配置化接入**（@张雨时 @张健）；预发/正式发布纳入同一阶段运维动作，**不单列 SOP 阶段 4**。  
2. 译评平台一期将提交候选版本，发布管理 必须能接收并发布。  
3. 多语种、多环境并行扩张，继续靠发版改 Prompt 不可持续。

### 3.3 一期边界

**发布管理模块负责**：语种壳子、Prompt 版本库、三环境发布、配置 API、最小 Web。  
**发布管理模块不负责**：写 Prompt、跑评测、PE 编辑、登录/审批、译配业务功能。
...
| KR | 指标 | 目标 |
|----|------|------|
| KR1 | 新语种 Prompt 上测试环境 | 零代码发版，发布管理 发布即可 |
| KR2 | 发布操作 | 预发/正式切换 ≤ 1 次 发布管理 操作 |
...
- **语种**：与 Prompt 评测共用 `language_id` 作为主键（API 历史字段名 `admin_language_id` 兼容）。
...
**发布管理**是**生产配置边界**：**Prompt 评测**回答「这版好不好」，**发布管理**回答「哪版在哪个环境生效」。
...
**发布管理**（`#/config`）模块；候选版本由 **Prompt 评测**「提交可应用」（M6）写入 **配置服务** 版本库。

```mermaid
flowchart TD
  A[Prompt 评测：新建语种并同步] --> B[Prompt 评测：提交可应用]
  B --> C[发布管理：自动入库 Prompt 版本]
  C --> D[发布管理：绑定测试环境并发布]
  D --> E[译配 test 拉配置]
  E --> F[发布管理：预发 / 正式发布]
  F --> G[发布管理：回滚 可选]
...
| A-1.2 | 语种注册（同步） | **不提供**发布管理 Web「新建」；由 **Prompt 评测** 创建时调用配置服务 `POST /v1/languages`（或迭代服务代调） | 同一 `language_id`；`display_name` 系统拼接；初始 `status=none`（UI：**无**）；列表自动可见 | P0 |
...
> 发布管理 **不设「草稿」阶段**。新建语种默认 `status=none`，界面展示 **无**（仅表示尚未进入测试/预发/正式流程）。与译评侧「Prompt 草稿 / 候选」（E-2.3）无关。
...
| A-2.2 | 版本查看 | 译评入库：三槽只读；**手工录入**：整段文本只读 | 与译评一致（分槽）；手工不分槽（见 A-2.5） | P0 |
...
| A-2.5 | 手工录入 | 详情页 **+ 新增 Prompt**：整段文本入库（**不分槽**）；来源 `manual` | 保存后立即可发布；原型 **P0** | P0 |
| A-2.6 | 版本 diff | 任意两版本对比 | 块级 diff | P2 |

**译评提交 · 自动入库（来自译评 E-6.1）**

与《PRD-Prompt 评测-一期》7.2.7 payload 对齐；服务端校验通过后**同步创建** `PromptVersion` 并返回（无需人工确认）：
...
| 做什么 | 说明 |
|--------|------|
| 本地缓存 | 按「语种 + 环境」缓存 Prompt，别每次翻译都请求 发布管理 |
...
- 发布管理 发布到 test 后，译配 **10 秒内**切到新 Prompt（走消息）  
...
**原型目录**：`AI 译评工作台/eval-prototype/`（`prototype/index.html` 跳转至 `#/config`）。
...
##### 语种注册（A-1.2 · 由 Prompt 评测同步）

发布管理 **不提供**「新建语种」按钮。语种在 **Prompt 评测**「新建语种」时写入配置服务，字段约定如下（与 Prompt 评测表单一致）：

| 字段 | 说明 |
|------|------|
| 源/目标语言 code | 必填；不可相同 |
| 负责人 / 备注 | 可选 |
| `display_name` | 系统拼接，Web 不手填 |
| `language_id` | `lang_{target_lang}_{source_lang}` |
| `status` | 初始 `none`（UI：**无**） |
...
4. **已入库 Prompt 版本**：右上角 **+ 新增 Prompt**（手工整段录入）；列表含译评自动入库与手工来源｜操作（查看 Prompt、评测报告）；译评「提交可应用」成功后**自动出现**，无待接收区  
...
| 查看 Prompt | 译评来源：三槽只读；手工来源：整段文本；完整撰写在 **Prompt 评测** |
...
| 方法 | 路径 | 调用方 | 说明 |
|------|------|--------|------|
| POST | `/v1/languages` | Prompt 评测 Web / BFF / 迭代服务 | 新建语种（同步配置服务；发布管理 Web **不调用**） |
...
> 下表是 **发布管理 产品一期 / 二期** 交付范围，不是 PRD 文档修订号。当前 PRD 文档为 **v1.3**。
...
| SOP 阶段 | 发布管理动作（工作台模块） |
|----------|---------------------------|
| 1 写 Prompt 0.5D | **Prompt 评测** 新建语种 → 配置服务同步（`status=none`） |
| 2 对比测试 1D | **Prompt 评测** 提交可应用 → **发布管理** Prompt 版本自动可见 |
| 3 测试接入 1D | **发布管理**：**test 发布**；验证通过后 **staging / production 发布 + 回滚**（不单列 SOP 阶段 4） |

### 8.5 待拍板项

- [x] ~~`gate_result` 与 发布管理 发布~~ → **已确认**：仅译评**专家标可用**可提交；发布管理 不展示 Gate（§0 #1）  
...
| v1.4 | 2026-06 | §0 Gate 改译评侧拦截；发布管理 Web 不展示；A-3.6 一期不做；发布表拆生效时间/操作人 |
...
| v1.13 | 2026-06 | 产品命名：**AI 译评工作台** / **Prompt 评测** / **发布管理** |
| v1.14 | 2026-06 | 三模块 IA：**人校评阅** 占位（无功能范围） |
| v1.15 | 2026-06 | §7.1 / §8.4 流程与 SOP 表对齐 **Prompt 评测** / **发布管理** 模块命名 |
| v1.16 | 2026-06 | 模块更名：**规则评测** → **Prompt 评测**（全文与流程图同步） |
| v1.17 | 2026-06 | §0 #11：发布管理去掉 Web「新建语种」；语种由 Prompt 评测创建并同步（A-1.2） |
| v1.18 | 2026-06 | 全文产品侧「Admin」改为「发布管理」；标题改为发布管理模块 |
| v1.19 | 2026-06 | 详情 **+ 新增 Prompt**（手工整段，不分槽）；A-2.5 升为 P0 |
