...
| 12 | 产品形态 | **一个工作台、三个模块（其中人校评阅一期仅占位）、两块后端**：本 PRD 能力归属 **Prompt 评测** + **迭代服务**；**人校评阅**、**发布管理**见双平台 §1；统一 IA 见《双平台功能列表》§1 |
...
**产品形态**：侧栏三模块 — **Prompt 评测**（`#/iter`）、**人校评阅**（`#/review`，一期仅占位）、**发布管理**（`#/config`）；分路由分页面，非同一详情 Tab。后端仍为 **迭代服务** + **配置服务**（见《双平台功能列表》§1）。
...
**原型目录**：`AI 译评平台 & Admin/eval-prototype/`（`prototype/` 跳转）— **侧栏三模块**、列表 + **分模块详情页**（无 Tab）。
...
*PRD v1.20 · 译配更新见 Admin PRD §7.2.4*
