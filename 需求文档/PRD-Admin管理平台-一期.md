...
> **文档版本**：v1.17 · 2026-06（PRD 修订号，见文末变更说明）  
...
| 9 | 产品形态 | **一个工作台、三模块（人校评阅一期仅占位）、两块后端**：本 PRD 能力归属 **发布管理** + **配置服务**；Prompt 评测/人校评阅见译评 PRD；统一 IA 见《双平台功能列表》§1 |
| 10 | 产品命名 | 工作台 **AI 译评工作台**；模块 **Prompt 评测** / **人校评阅**（占位）/ **发布管理**；本 PRD 对应 **发布管理** |
| 11 | 新建语种 | **发布管理 Web 不提供**「新建语种」；语种在 **Prompt 评测** 创建时由配置服务同步注册（同一 `language_id`），发布管理列表 **只读展示** |
...
**发布管理**（原 Admin）是**生产配置边界**：**Prompt 评测**回答「这版好不好」，**发布管理**回答「哪版在哪个环境生效」。
