...
> **文档版本**：v1.0 · 2026-06（PRD 修订号，见文末变更说明）  
> 关联文档：《一期功能清单》《双平台功能列表-一期》《PRD-译评平台-一期》  
> **文件路径**：`AI 译评平台 & Admin/需求文档/PRD-Admin管理平台-一期.md`  
> **交互原型**：`AI 译评平台 & Admin/eval-prototype/`（配置发布区块已合并；`prototype/` 跳转）
...
| 9 | 产品形态 | **一个工作台、两个模块、两块后端**：本 PRD 能力归属 **配置发布模块** + **配置服务**；撰写/评测见译评 PRD；统一 IA 见《双平台功能列表》§1 |

---

## 1. Summary

**配置发布模块**（原 Admin 管理平台）一期是**全新建设**，用于把 Prompt 从代码工程中解耦：管理语种生命周期、维护 Prompt 版本库、在**测试 / 预发 / 正式**三环境绑定并发布 Prompt，并通过配置 API 供 AI 译配运行时拉取。**一期不做登录与权限**（已确认），面向内网/VPN 可信环境。  
**产品形态**：**配置发布** 为独立应用（`#/config`），与 **质量迭代**（`#/iter`）分路由分页面；**左侧边栏**切换模块，品牌「语种工作台」仅在侧栏；主区顶栏仅原型提示。**配置服务** 与 **迭代服务** 分库分部署。
...
1. 新语种 SOP 阶段 3/4 明确 **Admin 配置化接入与上线**（@张雨时 @张健）。  
...
| 3 测试接入 1D | **test 环境发布** |
| 4 预发&正式 1D | **staging / production 发布 + 回滚** |
...
| v1.9 | 2026-06 | §0 #9：**一个工作台、两个模块、两块后端**；Summary 对齐语种工作台 |
