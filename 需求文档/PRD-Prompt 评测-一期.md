...
> 文档版本：v1.21 · 2026-06  
> 关联文档：《一期功能清单》《平台功能列表-一期》《PRD-发布管理模块-一期》  
> **交互原型**：`AI 译评工作台/eval-prototype/`（已与发布管理合并为 **AI 译评工作台**，见 §7.3）
...
AI 译评平台已具备**评测任务、自动评测、人校反馈**能力。一期在现有基础上补齐 **Prompt 撰写与版本化、分析报告 / 对比报告、专家标注可用、提交可应用** 等能力，使新语种能在平台内完成「写 Prompt → 跑评 → 看报告 → 人校 → 优化 → 专家标可用 → 提交可应用」闭环，支撑约 **3.5 天**新语种 SOP。
...
- **缺口**：Prompt 仍靠飞书/本地 Markdown；过线靠人读报告；优化问题散落人校过程；过线版本无法结构化提交可应用。

### 3.2 为什么现在做

1. 新语种 SOP 已收敛为 **4 阶段 / 约 3.5D**，阶段 2 明确要求「译评平台自动化」。
2. 发布管理模块一期同步建设，译评必须产出**可入库、可关联报告**的 Prompt 版本。
3. 德语等语种已有 system_v1 评测框架，可复制到新语种，瓶颈在 **Prompt 迭代闭环**而非重跑评测。

### 3.3 一期边界

**Prompt 评测模块（本 PRD）负责**：Prompt 迭代、评测、人校、分析/对比报告、专家标可用、提交入库。  
**不负责**：环境发布与 Config API（归 **发布管理模块 / 配置服务**）、登录权限、替代译配 PE 生产。  
**产品形态**：侧栏三模块 — **Prompt 评测**（`#/iter`）、**人校评阅**（`#/review`，一期仅占位）、**发布管理**（`#/config`）；分路由分页面，非同一详情 Tab。后端仍为 **迭代服务** + **配置服务**（见《平台功能列表功能列表》§1）。
...
| KR3 | 提交可应用 的版本 | 100% 携带评测报告 ID；且均为专家已标 **可用** |
...
> 当我要接入一个新语种时，我想在**同一个平台**里写完 Prompt、跑完评测、看完人校结论，并知道这版能不能交给 发布管理 发布，这样我就不用反复贴文档、猜报告、找研发改代码。
...
| 发布衔接 | 口头/手工同步 发布管理 | 一键提交，报告 ID 绑定 |

### 6.3 差异化

相比「只做离线 JSONL + HTML 查看器」，译评平台一期形成**可发布版本**的输出，与 发布管理 配置化发布闭环，而非仅作分析工具。
...
#### 7.1.2 Prompt 评测主流程（跨模块）

**AI 译评工作台** 内以 **Prompt 评测**（`#/iter`）为主链路；**新建语种** 时同步配置服务，**发布管理** 列表自动可见（发布管理 **无** 新建按钮）。人校一期在现网 **M5** 完成，侧栏 **人校评阅**（`#/review`）仅占位；专家标 **可用** 后向 **发布管理**（`#/config`）提交可应用（M6 / 配置服务）。

```mermaid
flowchart TD
  A[Prompt 评测：新建语种] --> B[创建语种规则 v1]
  B --> C[Prompt 评测：撰写 Prompt 三槽]
  C --> D[Prompt 评测：创建评测任务]
  D --> E[自动跑评 M4 · 已有]
  E --> R[分析报告 / 对比报告]
  R --> G[人校优化 Prompt · 现网 M5]
  G --> H[Prompt 评测：改语种规则 v2]
  H --> D
  R --> M{专家标可用?}
  M -->|是| I[提交至发布管理 M6]
  M -->|否| G
...
> **图注**：M4/M5/M6 为现网/接口能力编号；**人校评阅** 一期不交付功能，虚线表示规划模块。
...
|---------|------|------|----------|--------|
| E-1.1 | 新建语种并同步 | **Prompt 评测**「新建语种」→ 迭代服务写语种并 **同步配置服务**（同一 `language_id`）；发布管理列表自动出现 | 语对幂等；重复语对提示已存在；**发布管理无**新建按钮 | P0 |
| E-1.1b | 关联语种 | 创建/编辑评测任务时选择已注册 `language_id` | 任务详情展示语对；无语种时引导至 Prompt 评测新建 | P0 |
...
| 需求 ID | 功能 | 描述 | 验收标准 |
|---------|------|------|----------|
| E-2.0 | 统一块模型 | 所有入口读写同一 `blocks` 结构 | 与 发布管理 / 评测 JSONL 一致 |
| E-2.2 | 版本号 | v1/v2/v3 或 semver；changelog **可选** | 版本列表倒序；已提交可应用 的版本不可删 |
| E-2.3 | 版本状态 | **待评测 → 评测中 → 可用/不可用 → 已提交** | **可用 / 不可用** 由专家**人工切换**（需填写操作人姓名）；**不**由报告自动写入；**可用** 后可提交可应用 |
...
**提交可应用**：仅 `expert_usability = usable` 且非已提交版本可提交；payload 可带 `eval_report_id`、可选 `gate_result`（若未来跑批仍产出，仅作存档）。
...
| E-5.0 | 人校入口 | 迁入 **人校评阅** 模块（见 §0 #14）；Prompt 评测仅保留分析/对比**报告摘要** | 一期原型：人校评阅为占位页；现网 M5 不回归 | 后续 |
...
| 需求 ID | 功能 | 描述 | 验收标准 | 优先级 |
|---------|------|------|----------|--------|
| E-6.1 | 提交可应用 | POST 配置服务；携带语种 ID、版本号、各块内容、报告 ID；`gate_result` 可选 | 前置：**专家已标可用**；**自动入库**；失败可重试 | P0 |
| E-6.2 | 读发布管理生效版本 | GET 测试环境当前 Prompt version ID | 可发起「复验评测」任务 | P1 |
| E-6.3 | 导出 | JSONL、汇总报告、优化清单下载 | 与现有导出并存 | P1 |

**译评 → 发布管理 提交 payload（一期）**
...
**原型目录**：`AI 译评工作台/eval-prototype/`（`prototype/` 跳转）— **侧栏三模块**、列表 + **分模块详情页**（无 Tab）。

| 页面 | 路由 / 说明 | 原型 |
|------|-------------|------|
| 语种管理（Prompt 评测） | `#/iter` · **+ 新建语种**；列：语种、语对、**最新规则版本**、负责人；`← 语种管理` 返回列表 | ✅ |
| 语种管理（发布管理） | `#/config` · **无新建**（由 Prompt 评测同步）；列：语种、语对、**应用阶段**、已入库数、负责人 | ✅ |
| Prompt 评测详情 | `#/iter/lang/{id}` · 语种规则版本（**人工标可用**）/ 评测任务（**分析报告 / 对比报告**） | ✅ |
...
- 与 发布管理：REST JSON；内网调用；失败重试 + 幂等（`eval_prompt_version_id` 去重）。
...
| 仅 VPN 访问 | 外网误暴露 | 发布管理/Prompt 评测仅内网域名，不对公网开放 |
| 发布管理 API（配置服务）同期可用 | 提交阻塞 | 与 《PRD-发布管理模块-一期》 联调里程碑对齐 |
...
1. 选定 1 个语种，完成 Prompt v1 → 跑评 → 看分析/对比报告 → 现网人校 → v2 → **专家标可用** → 提交可应用 全链路。  
2. 提交 payload 在 发布管理 版本库可见，且 `eval_report_id` 正确；仅专家标 **可用** 的版本可提交。  
3. 阶段 2 SOP 在试点范围 **1 个工作日内**跑通（业务侧签字）。  
4. M1/M4/M5 已有功能无回归。

### 8.3 待拍板项

- [ ] Prompt 评测 ↔ 发布管理 联调里程碑日期  
- [x] 发布管理 **不校验 Gate**；仅 **pass / pass_with_warnings** 可提交可应用（见 《PRD-发布管理模块-一期》 §0 #1）
...
| v1.16 | 2026-06 | 产品命名：**AI 译评工作台** / **Prompt 评测** / **发布管理** |
| v1.17 | 2026-06 | 新增模块 **人校评阅**（`#/review`）· 一期仅 IA 占位，不交付功能 |
| v1.18 | 2026-06 | §7.1.2 主流程图对齐三模块命名（Prompt 评测 / 人校评阅占位 / 发布管理） |
| v1.19 | 2026-06 | 模块更名：**规则评测** → **Prompt 评测**（路由 `#/iter` 不变） |
| v1.20 | 2026-06 | §0 #15：语种仅在 Prompt 评测创建并同步发布管理；E-1.1 拆分同步与关联 |
| v1.21 | 2026-06 | 全文产品侧「Admin」改为「发布管理」 |
...
| 3 | 登录 / 权限 | **一期不做**；译评、发布管理 均内网/VPN 使用 |
...
| 6 | 提交可应用 前置 | 仅专家标为 **可用** 的版本可「提交可应用」；报告 ID 必填；`gate_result` 可选入库字段（**不驱动**可用状态） |
...
| 12 | 产品形态 | **一个工作台、三个模块（其中人校评阅一期仅占位）、两块后端**：本 PRD 能力归属 **Prompt 评测** + **迭代服务**；**人校评阅**、**发布管理**见平台功能列表 §1；统一 IA 见《平台功能列表功能列表》§1 |
| 13 | 产品命名 | 工作台 **AI 译评工作台**；模块 **Prompt 评测** / **人校评阅** / **发布管理**（`#/iter` / `#/review` / `#/config`） |
| 14 | 人校评阅模块 | **一期仅占位**（原型说明页），**不交付**结果详情/review 功能；规划承载逐条评阅 + 人工改分（见 E-5.x 后续） |
| 15 | 语种创建 | **仅在 Prompt 评测** Web 提供「新建语种」；创建时同步配置服务语种壳（`status=none`），**发布管理列表自动可见**，发布管理 **不提供** 新建入口 |


*PRD v1.0 · 译配更新见《PRD-发布管理模块-一期》§7.2.4*
